...
class Recorder():
  ...
  def record(self,
    ...
    with dai.Device(self.__pipeline) as device:
      ...
      rgbH265Path = outputDirPath.joinpath(f"{tag}rgb.{encoder}")
      leftH265Path = outputDirPath.joinpath(f"{tag}left.{encoder}")
      rightH265Path = outputDirPath.joinpath(f"{tag}right.{encoder}")
    ...
    if convertToMp4:
      ...
      convertor.convertAll(self.fps, processPaths)
